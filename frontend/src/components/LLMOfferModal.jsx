...
export default function LLMOfferModal({ isOpen, onClose, lead, onMarkContacted, openaiApiKey }) {
  ...
  // Reset state on open without wasting tokens
  useEffect(() => {
    if (lead && isOpen) {
      setErrorMsg("");
      setOfferText("");
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/85 backdrop-blur-sm p-3 sm:p-4 overflow-y-auto">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 my-auto">
        ...
        <div className="p-4 sm:p-5 space-y-4 max-h-[75vh] overflow-y-auto scrollbar-thin scrollbar-thumb-slate-800">
          ...
          {/* Generated Textbox or Empty State */}
          <div className="space-y-1.5">
            ...
            {!offerText && !loading ? (
              <div className="p-6 rounded-xl border border-dashed border-slate-800 bg-slate-950/40 text-center space-y-3">
                ...
                <div className="space-y-1">
                  <p className="text-xs font-semibold text-slate-300">
                    Оффер еще не сформирован
                  </p>
                  <p className="text-[11px] text-slate-500 max-w-sm mx-auto">
                    Токены не тратятся при простом открытии карточки. Нажмите кнопку генерации, когда будете готовы написать лиду.
                  </p>
                </div>
                <div className="flex flex-col sm:flex-row items-center justify-center gap-2 pt-1">
                  <button
                    type="button"
                    onClick={handleTriggerChatGpt}
                    className="px-4 py-2 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 shadow-md shadow-purple-600/20 transition-all"
                  >
                    <Zap className="w-3.5 h-3.5 text-amber-300 fill-amber-300" />
                    <span>Сгенерировать через ChatGPT</span>
                  </button>
                  <button
                    type="button"
                    onClick={() => handleToneSelect("business")}
              ...
              <textarea
                ...
                placeholder={loading ? "ChatGPT анализирует профиль и пишет персонализированное сообщение..." : ""}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between p-4 bg-slate-950/70 border-t border-slate-800 gap-2.5">
          ...
          <div className="flex items-center space-x-2 w-full sm:w-auto order-1 sm:order-2">
            <button
              onClick={() => handleCopy()}
              disabled={!offerText}
              className="flex-1 sm:flex-initial px-4 py-2 bg-slate-800 hover:bg-slate-700 disabled:opacity-40 disabled:cursor-not-allowed text-slate-200 rounded-xl text-xs font-semibold flex items-center justify-center space-x-1.5 transition-colors"
            >
            ...
            <button
              ...
              disabled={!offerText}
              className="flex-1 sm:flex-initial px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 disabled:opacity-40 disabled:cursor-not-allowed text-white rounded-xl text-xs font-semibold flex items-center justify-center space-x-1.5 shadow-md shadow-blue-600/20 transition-all"
            >
